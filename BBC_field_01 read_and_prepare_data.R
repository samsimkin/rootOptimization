...
startDate_checked <- "2012-01"
endDate_checked <- "2024-06"
...
# write.table(bbc_percore, "bbc_percore.txt", sep = "\t", row.names = F) # exported 2024-11-14
# write.table(bbc_rootmass, "bbc_rootmass.txt", sep = "\t", row.names = F) # exported 2024-11-14
# write.table(bbc_rootChemistry, "bbc_rootChemistry.txt", sep = "\t", row.names = F) # exported 2024-11-14

# bbc_percore <- read.table("bbc_percore.txt", sep = "\t", header = TRUE)
# bbc_rootmass <- read.table("bbc_rootmass.txt", sep = "\t", header = TRUE)
# bbc_rootChemistry <- read.table("bbc_rootChemistry.txt", sep = "\t", header = TRUE)

# rootSampleArea unit is squareMeter
# dryMass unit is gram

bbc_percore_mod <- bbc_percore #%>% filter(rootSampleDepth > 24 & rootSampleDepth < 36 & siteID != "BARR" & siteID != "HEAL") 
 # BARR and HEAL already have just one clipID per plot, and just one coreID per clipID
 # nominal depth is 30 cm, so filtering to the above constrains to something somewhat close to that

# NLCD <- read.delim("All_NEON_TOS_Plot_Points_V10.csv", header = TRUE, sep = ",") %>% select(plotID, nlcdClass) %>% distinct()
# bbc_percore_mod <- merge(bbc_percore_mod, NLCD, by = "plotID", all.x = TRUE)
bbc_percore_mod$year <- substr(bbc_percore_mod$eventID,10,13)

bbc_percore_mod <- bbc_percore_mod %>% filter(sampleID != "BBC.YELL055671.20200813.NORTH") # remove the one record from this bout
...
# write.table(stature_year, "stature_year.txt", sep = "\t", row.names = F) # exported 2024-07-20

#bbc_percore_mod <- merge(bbc_percore_mod, stature_year, by = c("siteID"), all.x = T)
...
#bbc_percore_mod$sampleID_simple <- substr(bbc_percore_mod$sampleID, 1,23)

bbc_rootChemistry_mod <- bbc_rootChemistry 
...
bbc_rootChemistry_clipMean <- bbc_rootChemistry_mod %>% filter(!is.na(nitrogenPercent)) %>% group_by(sampleID_simple) %>% summarize(nitrogenPercent_ave = mean(nitrogenPercent)) 
  # get the average of all cores and sizes within a clipID on a particular date

bbc_rootChemistry_lumped <- bbc_rootChemistry 
...
bbc_rootMass_grouped <- bbc_rootMass_grouped %>% group_by(sampleID, sampleID_simple, sizeCategory) %>% summarise(dryMass = sum(dryMass))  # add dead and live root masses, and combine old 0-05 and 05-1 sizeCategories
...
# bbc_rootMass_NS <- bbc_rootMass_grouped %>%  group_by(sampleID_simple, sizeCategory) %>% summarise(dryMass = mean(dryMass)) %>% ungroup()  # calc ave of north and south cores
# bbc_rootMass_and_chem <- bbc_rootMass_NS %>% left_join(bbc_rootChemistry_mod, by = c("sampleID_simple","cnSampleID")) %>% left_join(bbc_rootChemistry_clipMean, by = "sampleID_simple")
# bbc_rootMass_and_chem$nitrogenPercent <- ifelse(is.na(bbc_rootMass_and_chem$nitrogenPercent), bbc_rootMass_and_chem$nitrogenPercent_ave, bbc_rootMass_and_chem$nitrogenPercent)
# #bbc_rootMass_and_chem <- bbc_rootMass_and_chem %>% rename(sampleID = sampleID_simple) 
# 

# bbc_rootmass_sum <- bbc_rootMass_and_chem %>% left_join(bbc_rootmass_sum, by = "sampleID_simple") %>% mutate(mass_per = dryMass/dryMassTot, nitrogen_weight = nitrogenPercent * mass_per) %>% 
#   group_by(sampleID_simple) %>% summarise(dryMassTot = sum(dryMass), nitrogenPercent = sum(nitrogen_weight))

#bbc_rootmass_sum <- bbc_rootmass %>% filter(qaDryMass == "N") %>% group_by(sampleID) %>% summarise(dryMassTot = sum(dryMass))
...
# data_total$mass_fract <- data_total$nitrogen_weight <- NA
...
#write_delim(data_merge, "data_merge.txt", delim = "\t")
...
#data_merge_lumped <- data_merge_lumped %>% mutate(nitrogen_weight = nitrogenPercent * mass_fract) 
data_total$cnSampleID <- NA
  # add nitrogen percent and then weight that nitrogen value by the fraction of core mass contributed by size category
...
#write_delim(data_merge_lumped, "data_merge_lumped.txt", delim = "\t")
